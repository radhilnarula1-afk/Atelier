...
import { signInWithGooglePopup, hasFirebaseConfig } from "@/lib/firebase";
...
function AuthScreen({ onAuth }: { onAuth: () => void }) {
  ...
  const [username, setUsername] = useState(""); // internally holds email
  ...
  const handleGoogleSignIn = async () => {
    if (!hasFirebaseConfig) {
      toast.error(
        "Google Sign-In is not configured yet. Please fill in the VITE_FIREBASE_ environment variables in your frontend/.env.local file."
      );
      return;
    }
    
    setGoogleLoading(true);
    const loadId = toast.loading("Connecting with Google...");
    try {
      ...
      toast.success(`Welcome to Atelier, ${res.username}!`);
      onAuth();
    } catch (err: any) {
      toast.dismiss(loadId);
    ...
    <div className="min-h-screen flex items-center justify-center bg-surface grain px-6">
      <div className="w-full max-w-md p-10 bg-background rounded-sm shadow-editorial border border-border/40">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            <label className="text-[10px] uppercase tracking-[0.2em] text-muted-foreground block mb-2">Email Address</label>
            <input
              type="email"
              required
          ...
          <button
            ...
            className="w-full bg-foreground text-background py-3.5 text-sm uppercase tracking-[0.1em] hover:bg-foreground/90 transition disabled:opacity-50 font-medium"
          >
        ...
        <div className="relative my-6">
          ...
          <div className="relative flex justify-center text-xs uppercase">
            <span className="bg-background px-3 text-muted-foreground text-[9px] tracking-[0.25em]">or</span>
          </div>
        </div>
        ...
        <button
          ...
          disabled={loading || googleLoading}
          className="w-full bg-surface border border-border py-3 text-sm tracking-[0.05em] hover:bg-muted transition flex items-center justify-center gap-3 font-medium disabled:opacity-50"
        >
